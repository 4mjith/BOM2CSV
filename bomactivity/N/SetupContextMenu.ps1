...
$scriptDirectory = "C:\bomactivity\N"
$scriptPath = Join-Path $scriptDirectory "BomCsvConverter.ps1"
...
# Save the converter script to the specified location
$converterScript = @'
param(
...
Set-Content -Path $scriptPath -Value $converterScript -Force
Write-Host "Created script file: $scriptPath"

# Registry modifications
try {
    # Remove existing registry entries first
    $registryPaths = @(
        "Registry::HKEY_CURRENT_USER\Software\Classes\.csv\shell\AddBOMToCSV",
        "Registry::HKEY_CURRENT_USER\Software\Classes\Excel.CSV\shell\AddBOMToCSV",
        "Registry::HKEY_CURRENT_USER\Software\Classes\CSVFile\shell\AddBOMToCSV"
    )
    ...
    foreach ($basePath in @(
        "Registry::HKEY_CURRENT_USER\Software\Classes\.csv\shell",
        "Registry::HKEY_CURRENT_USER\Software\Classes\Excel.CSV\shell",
        "Registry::HKEY_CURRENT_USER\Software\Classes\CSVFile\shell"
    )) {
        if (-not (Test-Path $basePath)) {
            New-Item -Path $basePath -Force | Out-Null
        }

        $commandPath = "$basePath\AddBOMToCSV"
        New-Item -Path $commandPath -Force | Out-Null
        
        New-ItemProperty -Path $commandPath -Name "MUIVerb" -Value "Add BOM to CSV" -PropertyType String -Force | Out-Null
        New-ItemProperty -Path $commandPath -Name "Icon" -Value "shell32.dll,23" -PropertyType String -Force | Out-Null
        # Add property to show in selection
        New-ItemProperty -Path $commandPath -Name "MultiSelectModel" -Value "Player" -PropertyType String -Force | Out-Null

        $execPath = "$commandPath\command"
        New-Item -Path $execPath -Force | Out-Null
        $commandValue = "powershell.exe -NoProfile -ExecutionPolicy Bypass -File `"$scriptPath`" -FilePaths `"%1`""
        New-ItemProperty -Path $execPath -Name "(Default)" -Value $commandValue -PropertyType String -Force | Out-Null
    }

    [System.Windows.Forms.MessageBox]::Show("Context menu entry added successfully! Please log out and log back in for changes to take effect.", "Success", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
}
catch {
    [System.Windows.Forms.MessageBox]::Show("Error setting up context menu: $_", "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
}